...
//Однако первое значение функции reduce - аккумулятор (accumulator), в который сводится массив. Остальная часть
// аргументов вполне ожидаема: текущий элемент массива, текущий индекс и сам массив. Методу reduce передается (
// необязательно) начальное значение для аккумулятора.
// Например, суммирование чисел, хранящихся в массиве, или вычисление их среднего являются способами свести массив к
// единому значению.

//Суммирование чисел в массиве, используя начальное значение
...
/* Передаваемая в reduce функция получает два параметра: аккумулятор (а) и текущий элемент массива (х).
В этом примере аккумулятор изначально содержит значение О. Поскольку это наш первый опыт с reduce, рассмотрим все
этапы, которые проходит JavaScript, чтобы лучше понять, как это работает.
    1. Для первого элемента массива (5) вызывается (анонимная) функция. Первоначальна, а имеет значение 0,
    а х - значение 5. Функция возвращает сумму а и х (5), что становится значением а на следующем этапе.

    2. Функция вызывается для второго элемента массива (7). Теперь а имеет значение 5 (переданное с предыдущего этапа),
    а х имеет значение 7 . Функция возвращает сумму а и х (12), которая становится значением а на следующем этапе.

    3. Функция вызывается для третьего элемента массива (2). Теперь а имеет значение 12, а х - значение 2.
    Функция возвращает сумму а и х (14).

    4. Функция вызывается для четвертого, и последнего, элемента массива (4). Теперь а имеет. значение 14, а х -
    значение 4. Функция возвращает сумму а и х (18), которая и будет возвращаемым значением функции reduce
    (которое затем присваивается константе sum).
*/

//Суммирование чисел в массиве, не используя начальное значение

const z14 = [5,7,2,4];
const summ = z14.reduce((a,x) => a += x);
console.log(summ);

/*
Прежде чем мы перейдем к более интересным случаям применения метода reduce, давайте рассмотрим, что будет, если
аккумулятору не присваивается начальное значение, т.е. он равен unde f ined. Тогда метод reduce считает первый
элемент массива в качестве начального значения и начинает вызывать функцию со вторым элементом. Давайте вернемся к
нашему примеру, но опустим начальное значение.

    1. Для второго элемента массива ( 7 ) вызывается (анонимная) функция. У а теперь начальное значение
    5 (первый элемент массива), а х содержит значение 7. Функция возвращает сумму а и х (12), что становится
    значением а на следующем этапе.

    2. Функция вызывается для третьего элемента массива (2). Теперь а имеет начальное значение 12, а х - значение 2.
    Функция возвращает сумму а и х (14).

    3. Функция вызывается для четвертого, и последнего, элемента массива (4 ). Теперь а имеет значение 14,
    а х - значение 4. Функция возвращает сумму а и х (18), которая и становится возвращаемым значением reduce
    (оно затем присваивается константе sum).

Как можно заметить, здесь на один этап меньше, но результат то же. В этом примере (и в любом другом случае, когда
первый элемент может служить начальным значением аккумулятора) мы можем извлечь пользу, исключив начальное значение.

Обычно для метода reduce в качестве аккумулятора используется значение базового типа (число или строка), но
использование для аккумулятора объекта - это очень мощный подход (о котором часто забывают). Например, если у вас есть
массив строк и вы хотите сгруппировать строки в упорядоченные по алфавиту массивы (слова на А, слова на Б и т.д.),
можете использовать объект.
*/

const words = ["Beachball", "Rodeo", "Angel", "Aardvark", "Xylophone", "NovemЬer", "Chocolate", "Рарауа", "Uniform",
    "Joker", "Clover", "Bali"];
